...
<div class="layout-px-spacing">
    <div class="tm-page">
        <div class="tm-page-head">
            <div><h1 class="tm-title">Archive</h1><div class="tm-crumb"><a href="{{ url('dashboard') }}">Home</a> / Archive / Communications</div></div>
        </div>

        @include('archive.partials.tabs', ['active' => 'communications'])

        <div class="tm-card">
            <table id="zero-config" class="tm-table" style="width:100%">
                <thead>
                    <tr><th>Date</th><th>Type</th><th>Source / Addressee</th><th>Particulars</th><th>Actions Taken</th><th>Received / Released By</th><th class="no-content">Actions</th></tr>
                </thead>
                <tbody>
                    @foreach($records as $item)
                        @php
                            $isOut = strtoupper($item->communication_type ?? '') == 'OUTGOING' || !empty($item->date_released);
                            $theDate = $item->date_received ?: $item->date_released;
                            $party = $item->source ?: $item->addressee;
                            $person = $item->received_by ?: $item->released_by;
                        @endphp
                        <tr data-id="{{ $item->id }}">
                            <td>{{ $theDate ? date("M d, Y", strtotime($theDate)) : '—' }}</td>
                            <td><span class="tm-badge {{ $isOut ? 'tm-badge-sec' : 'tm-badge-info' }}">{{ $isOut ? 'OUTGOING' : 'INCOMING' }}</span></td>
                            <td>{{ $party }}</td>
...
<script>
    tmInitDataTable('#zero-config', { pageLength: 10, searchPlaceholder: 'Search archive...', order: [[0, 'desc']], columnDefs: [{ orderable: false, targets: [6] }] });
    (function () {
        var menu = document.getElementById('tm-row-menu'); if (!menu) return;
        var curId = null, viewBase = "{{ url('communications/view') }}", unarchiveBase = "{{ url('archive/communications_unarchive') }}";
