    ...
    <div class="tm-page">
        <div class="tm-page-head">
            <div>
                ...
                <div class="tm-crumb"><a href="{{ url('dashboard') }}">Home</a> / Archive / Resolutions</div>
            </div>
        </div>

        @include('archive.partials.tabs', ['active' => 'resolutions'])

        <div class="tm-card">
            <table id="zero-config" class="tm-table" style="width:100%">
                <thead>
                    <tr>
                        <th>Series No.</th><th>Title</th><th>Author</th><th>Keywords/Tags</th><th>Sponsor</th>
                        <th>Date Created</th><th>Attested By</th><th>Recorded By</th><th>Approved By</th><th>Approved Date</th>
                        <th>Book Ref No.</th><th>Page Ref No.</th><th>Total Pages</th><th>Minute Ref No.</th><th class="no-content">Actions</th>
                    </tr>
                </thead>
                <tbody>
...
<script>
    tmInitDataTable('#zero-config', { pageLength: 10, searchPlaceholder: 'Search archive...', order: [[5, 'desc']], columnDefs: [{ orderable: false, targets: [14] }, { type: 'tm-series', targets: [0] }] });

    (function () {
        var menu = document.getElementById('tm-row-menu'); if (!menu) return;
        var curId = null;
        var viewBase = "{{ url('resolutions/view') }}", unarchiveBase = "{{ url('archive/resolutions_unarchive') }}";
        function hideMenu() { menu.style.display = 'none'; }
        $('#zero-config tbody').on('click', 'tr', function (e) {
            if ($(e.target).closest('a, .tm-actions, button, input').length) return;
            curId = $(this).data('id'); if (!curId) return;
        ...
        $(menu).on('click', '[data-act]', function () {
            var act = this.getAttribute('data-act'); hideMenu();
            if (act === 'view') window.location.href = viewBase + '/' + curId;
            else if (act === 'unarchive') confirm_unarchive(unarchiveBase + '/' + curId);
        });
    ...
    function confirm_unarchive($url) {
        Swal.fire({ title: 'Are you sure?', text: "This will remove the record from the archive list.", icon: 'warning', showCancelButton: true, confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Yes, remove it!' })
