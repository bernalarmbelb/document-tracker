...
<div class="layout-px-spacing">
    <div class="tm-page">
        ...
        <div class="tm-page-head">
            <div>
                <h1 class="tm-title">Outgoing Communications</h1>
                <div class="tm-crumb"><a href="{{ url('communications/outgoing_list') }}">Communications</a> / Outgoing</div>
            </div>
            <div class="d-flex align-items-center" style="gap:10px; flex-wrap:wrap">
                ...
                @endif
                <a href="{{ url('communications/print_outgoing') }}" class="tm-btn tm-btn-outline" title="Export list">
                ...
                <tbody>
                    @foreach($records as $item)
                        <tr data-id="{{ $item->id }}" data-title="{{ $item->particulars }}">
                            <td>{{ date("M d, Y", strtotime($item->date_released)) }}</td>
          ...
          <div class="simple-pill">
              <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                  <li class="nav-item" role="presentation">
                    <button class="nav-link" id="pills-incoming-tab" data-bs-toggle="pill" data-bs-target="#pills-incoming" type="button" role="tab" aria-controls="pills-incoming" aria-selected="false">
                  ...
                  <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="pills-outgoing-tab" data-bs-toggle="pill" data-bs-target="#pills-outgoing" type="button" role="tab" aria-controls="pills-outgoing" aria-selected="true">
              ...
              <div class="tab-content" id="pills-tabContent">
                  <div class="tab-pane fade" id="pills-incoming" role="tabpanel" aria-labelledby="pills-incoming-tab" tabindex="0">
                  ...
                  <div class="tab-pane fade show active" id="pills-outgoing" role="tabpanel" aria-labelledby="pills-outgoing-tab" tabindex="0">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
                    <form class="row g-3" action="{{ url('/communications/save_changes_outgoing/') }}" method="post" autocomplete="off">
                        ...
                        <div class="col-12"><label class="tm-label">Date Released <span class="text-danger">*</span></label><input type="date" class="tm-input" value={{ date("Y-m-d") }} required id="edit_date_released" name="date_released"></div>
                        <div class="col-12"><label class="tm-label">Addressee <span class="text-danger">*</span></label><input type="text" class="tm-input" required id="edit_addressee" name="addressee" ></div>
                        ...
                        <div class="col-12"><label class="tm-label">Person who released <span class="text-danger">*</span></label><input type="text" class="tm-input" required id="edit_released_by" name="released_by" ></div>
                        <div class="d-grid gap-2 col-12 mx-auto"><button type="submit" name="btnsave" value="1" class="tm-btn tm-btn-primary tm-btn-block">Save</button></div>
                    </form>
          </div>
      </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="file" class="filepond" id="filepond-outgoing" name="myfile" multiple>
...
<script>
    ...
    function view_details(param) {
        $.ajax({
            ...
            url: '{{ url("communications/get_outgoing_info") }}',
            success: function (data) {
                document.getElementById('edit_communication_id').value = data['info']['id'];
                document.getElementById('edit_date_released').value = data['info']['date_released'];
                document.getElementById('edit_addressee').value = data['info']['addressee'];
                document.getElementById('edit_particulars').value = data['info']['particulars'];
                document.getElementById('edit_released_by').value = data['info']['released_by'];
            },
            error: function (XHR, textStatus, errorThrown) { console.log(errorThrown); console.log(XHR.responseText); }
        });
    }

    function refreshUploadedFilesOutgoing(param) {
        $.ajax({
            type: 'POST', data: { communication_id: param }, dataType: "json",
            url: '{{ url("communications/get_outgoing_files") }}',
            success: function (data) {
                let items = data['rows'];
                ...
                items.forEach(item => {
                    ...
                    b.href = "{{ url('communications/delete_uploaded_file_outgoing') }}/" + item.id;
    ...
    let filepondOutgoing = null;

    function initFilePondOutgoing(resId) {
        const input = document.getElementById('filepond-outgoing');
        if (filepondOutgoing) {
            filepondOutgoing.destroy();
        }
        filepondOutgoing = FilePond.create(input, {
            allowMultiple: true,
            storeAsFile: true,
            credits: false,
            labelIdle: `
            ...
            `,
            server: {
                process: '{{ url("/communications/upload_supporting_documents_outgoing") }}?communication_id=' + resId,
                headers: {
            ...
            onprocessfile: function (error, file) {
                if (!error) {
                    refreshUploadedFilesOutgoing(resId);
                }
            }
        });
    }
    ...
    function view_files(param, paramparticulars) {
        ...
        refreshUploadedFilesOutgoing(param);
        initFilePondOutgoing(param);
    }
</script>
@endsection
